...
COMMON_REPO ?= $(shell bash -c 'export MK_PATH=$(MK_PATH); echo $${MK_PATH%mm2/*}')
...
CXXFLAGS += -I$(XF_PROJ_ROOT)/common/includes/xcl2 -I/opt/Xilinx/Vitis_HLS/2021.2/include
...
VPP_FLAGS += --connectivity.sp mm_1.varND_off:DDR[0]
VPP_FLAGS += --connectivity.sp mm_1.clsK_off:DDR[1]
VPP_FLAGS += --connectivity.sp mm_1.ClausesVec:DDR[2]
VPP_FLAGS += --connectivity.sp mm_1.VarsLocVec:DDR[3]
...
ifeq ($(HOST_ARCH), x86)
	...
	$(VPP) -p $(BUILD_DIR)/mm.link.xclbin -t $(TARGET) --platform $(DEVICE) --package.out_dir $(PACKAGE_OUT) -o $(BUILD_DIR)/mm.xclbin
else
...
ifneq ($(HOST_ARCH), x86)
	$(VPP) $(VPP_PFLAGS) -p $(BUILD_DIR)/mm.xclbin -t $(TARGET) --platform $(DEVICE) --package.out_dir $(PACKAGE_OUT) --package.rootfs $(EDGE_COMMON_SW)/rootfs.ext4 --package.sd_file $(SD_IMAGE_FILE) --package.sd_file xrt.ini --package.sd_file $(RUN_APP_SCRIPT) --package.sd_file $(EXECUTABLE) -o mm.xclbin
endif
