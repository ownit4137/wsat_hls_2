...
int main(int argc, char** argv) {
	...
	fileName = "/home/centos/src/project_data/aws-fpga/Vitis/examples/xilinx_2021.2/wsat_onboard/src/k3-r4.26-v600-c2556-043.cnf";
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_yalsat = cl::Kernel(program, "yalsat", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_yalsat.setArg(6, maxflip));
    OCL_CHECK(err, err = krnl_yalsat.setArg(7, issolved));
    OCL_CHECK(err, err = krnl_yalsat.setArg(8, seed));
	...
	std::cout << "Time: " << exec_time*1e-9 << ", flips: " << maxflip << std::endl;
	...
	if(issolved){
		...
		printf("Solver could not find a solution.\n");
	}
